...
- The existing v1 `.slasher` runner is only a temporary migration source.
...
4. `migration-from-slasher-v1.md`
   - Migration guide from existing `.slasher` files to `.numa`.
   - Owns rewrite patterns and porting guidance.
...
| Extension | Meaning |
|---|---|
| `.slasher` | Current v1 command-runner scripts. Temporary during the transition. |
| `.numa` | Future Numadora implementation scripts. |
...
- Existing `.slasher` scripts may stop working after the Numadora path is
  stable.
- Compatibility sugar should not steer the language design.
- Cross-calling between `.slasher` and `.numa` scripts is not part of the core
  model. Shared behavior should move into Numadora modules and host APIs.
...
6. Port important samples to `.numa`.
7. Remove or archive the v1 `.slasher` runner once `.numa` covers the core loop.
